import { NavLink, useNavigate } from 'react-router-dom'
import { cn } from '../../lib/utils'

const NAV_ITEMS = [
  { to: '/',           icon: '🏠' },
  { to: '/finances',   icon: '📊' },
  { to: '/game',       icon: '🎮' },
  { to: '/knowledge',  icon: '📚' },
]

export function BottomNav() {
  const navigate = useNavigate()

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-2 pt-1"
      style={{ background: 'linear-gradient(to top, var(--color-bg) 75%, transparent)' }}
    >
      <div className="flex items-center justify-around max-w-sm mx-auto">
        {NAV_ITEMS.slice(0, 2).map(({ to, icon }) => (
          <NavItem key={to} to={to} icon={icon} />
        ))}

        {/* Add transaction button */}
        <button
          onClick={() => navigate('/add-transaction')}
          className="w-14 h-14 rounded-full bg-green text-bg text-3xl font-bold flex items-center justify-center shadow-lg active:scale-95 transition-transform"
        >
          +
        </button>

        {NAV_ITEMS.slice(2).map(({ to, icon }) => (
          <NavItem key={to} to={to} icon={icon} />
        ))}
      </div>
    </nav>
  )
}

function NavItem({ to, icon }: { to: string; icon: string }) {
  return (
    <NavLink
      to={to}
      end={to === '/'}
      className={({ isActive }) =>
        cn(
          'flex items-center justify-center w-12 h-12 rounded-xl transition-all',
          isActive ? 'bg-green/10' : 'opacity-45',
        )
      }
    >
      <span className="text-2xl leading-none">{icon}</span>
    </NavLink>
  )
}
